...
try{const entry=resolve(temp,'entry.mjs');await writeFile(entry,['button','icon','badge','dialog','segmented-control','search-input','link','navigation','card','select','checkbox','accordion-item','alert'].map(x=>`import ${JSON.stringify(resolve(dir,'packages/elements/dist/define/'+x+'.js'))};`).join('\n'));
