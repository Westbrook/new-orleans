...
Places can be filtered by performance date, neighborhood, category and saved person/state. Performance dates select venues with dated or clearly marked weekly listings, not assigned itinerary stops. Sort by guide order, name, neighborhood or approximate distance from Hotel Fontenot. Picker choices stay on this device across refreshes: neighborhood, performance date, sort order, saved person/overlap, category, Map Show and Area, and itinerary day. Search text is retained too. Explicit links override only the picker they address; other cached choices remain intact. Invalid or unavailable cached options fall back safely, and Reset filters saves the cleared Places filters. Preferences continue to work for the current session if browser storage is unavailable. Search accepts accents, apostrophes and multiple words. Map numbers match the list; saved places have their own map view. Venue dialogs show performance details, ticket times and listening links, with a maximum height of 70dvh. Card titles link directly to venue websites. The checklist progress reflects only boxes checked on this device.
